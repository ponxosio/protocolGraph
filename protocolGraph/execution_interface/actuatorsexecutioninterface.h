...
public:
    ...
    virtual void startMeasureOD(const std::string & sourceId, units::Time duration, units::Frequency measurementFrequency, units::Length wavelength) = 0;
    virtual double getMeasureOD(const std::string & sourceId) = 0;

    virtual void startMeasureTemperature(const std::string & sourceId, units::Time duration, units::Frequency measurementFrequency) = 0;
    virtual units::Temperature getMeasureTemperature(const std::string & sourceId) = 0;

    virtual void startMeasureLuminiscense(const std::string & sourceId, units::Time duration, units::Frequency measurementFrequency) = 0;
    virtual units::LuminousIntensity getMeasureLuminiscense(const std::string & sourceId) = 0;

    virtual void startMeasureVolume(const std::string & sourceId, units::Time duration, units::Frequency measurementFrequency) = 0;
    virtual units::Volume getMeasureVolume(const std::string & sourceId) = 0;

    virtual void startMeasureFluorescence(const std::string & sourceId, units::Time duration, units::Frequency measurementFrequency) = 0;
